function walking(steps, footPrint, speed) {
    let distanceMetters = steps * footPrint;
    let speedMettersPerSecond = speed / 3.6;
    let time = distanceMetters / speedMettersPerSecond;
    let rest = Math.floor(distanceMetters / 500);
    let timeMin = Math.floor(time / 60);
    let timeSec = Math.round(time - (timeMin * 60));
    let timeHours = Math.floor(time / 3600);
    console.log((timeHours < 10 ? "0" : "") + timeHours + ":" + (timeMin + rest < 10 ? "0" : "") + (timeMin + rest) +  ":" + (timeSec < 10 ? "0" : "") + timeSec)
}
walking(4000, 0.60, 5)
walking(2564, 0.70, 5.5)